...
// Library scan types: 0-127 reserved for future library expansion
    ...
    HADAMARD_TYPE   = 1,
    SLEW_TYPE		= 2,
    ILLEGAL_SCAN_TYPE = -5, // This is to follow ERR_DLPSPEC_ILLEGAL_SCAN_TYPE of DLPSPEC_ERR_CODE
...
{
    patNormal,
    patEvenOddInterlaced,
    patHalfHoppingInterlaced,
    patMaxOrder
}patScrambleType;
...
DLPSPEC_ERR_CODE dlpspec_scan_interpret(const void *pBuf, const size_t bufSize,
	   	scanResults *pResults, patScrambleType patType);
...
DLPSPEC_ERR_CODE dlpspec_scan_interpReference(const void *pRefCal, 
		...
		const scanResults *pScanResults, scanResults *pRefResults, patScrambleType patType);
int32_t dlpspec_scan_genPatterns(const uScanConfig* pCfg, 
		const calibCoeffs *pCoeffs, const FrameBufferDescriptor *pFB, patScrambleType patType);
