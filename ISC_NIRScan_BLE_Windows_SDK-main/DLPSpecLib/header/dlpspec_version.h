...
// Version format: MAJOR.MINOR.BUILD.ISC
 ...
 * 2.1.1
    - Fix patHalfHoppingInterlaced latest pattern did not hopping.

 * 2.1.0
	- Change pattern generation in SA to width 6 and step 2 with 101 DMD segments
	- and control the generation range on DMD.

 * 2.0.5
    - Support patterns scrambled scan for even/odd and half-patterns jumping.

 * 2.0.4
    - Modify check peak distribution function.
    - Modify parameter prototype in getFWHM().
    - Update TMS470_18.1.

 * 2.0.3.3
    - Remove round() for shift vectors calculation.
    - Changed detector alignment patterns to be half two original patterns + 213 DMD segments.

 * 2.0.3
    - Interpolation function added: dlpspec_interpolate_double_positions().
    - Corrected issue with truncation of pointer arithmetic in 64-bit systems.

 * 2.0.2
    - DLL build script added.

 * 2.0.0
    - Public release since 1.1.5. See below for new features and bug fixes.
    ...
    - Added validation to ensure that reference being interpreted fully covers the scan config it is being interpreted to.
 ...
 * 1.1.4
    ...
    - pack the associated struct more tightly than the source system that serialized the data.

 * 1.1.3
    - Column scan generation speed increased for parity with Hadamard pattern generation.
