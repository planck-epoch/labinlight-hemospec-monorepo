        ...
        public static string GetServiceName(GattDeviceService service)
        {
            if (IsSigDefinedUuid(service.Uuid))
            {
                ...
                if (Enum.TryParse(Utilities.ConvertUuidToShortId(service.Uuid).ToString(), out serviceName))
                {
                    ...
                    return "Unknown Service: " + service.Uuid;
            }
            else
            {
                string serviceUUID = service.Uuid.ToString().ToUpperInvariant();

                foreach (KeyValuePair<string, string> d_services in ISC_UUID.Services.Services_UUID_Data_Set)
                {
                    if (d_services.Value == serviceUUID)
                        return d_services.Key + " Service";
                }
                return "Unknown Service: " + service.Uuid;
            }
        }

        public static string GetCharacteristicName(GattDeviceService service, GattCharacteristic characteristic, out bool deprecated)
        {
            deprecated = false;
            ...
            {
                string serviceUUID = service.Uuid.ToString().ToUpperInvariant();
                string characteristicUUID = characteristic.Uuid.ToString().ToUpperInvariant();

                foreach (KeyValuePair<string, string> d_services in ISC_UUID.Services.Services_UUID_Data_Set)
                {
                    if (d_services.Value == serviceUUID)
                    {
                        Dictionary<string, string> char_UUID_Set = new Dictionary<string, string>();
                        if (d_services.Value == ISC_UUID.Services.General_Information_UUID)
                            char_UUID_Set = ISC_UUID.General_Information_Service.Characteristics_UUID_Data_Set;
                        else if (d_services.Value == ISC_UUID.Services.Command_UUID)
                            char_UUID_Set = ISC_UUID.Command_Service.Characteristics_UUID_Data_Set;
                        else if (d_services.Value == ISC_UUID.Services.Current_Time_UUID)
                            char_UUID_Set = ISC_UUID.Current_Time_Service.Characteristics_UUID_Data_Set;
                        else if (d_services.Value == ISC_UUID.Services.Calibration_UUID)
                            char_UUID_Set = ISC_UUID.Calibration_Service.Characteristics_UUID_Data_Set;
                        else if (d_services.Value == ISC_UUID.Services.Configuration_UUID)
                            char_UUID_Set = ISC_UUID.Configuration_Service.Characteristics_UUID_Data_Set;
                        else if (d_services.Value == ISC_UUID.Services.Scan_Data_UUID)
                            char_UUID_Set = ISC_UUID.Scan_Data_Service.Characteristics_UUID_Data_Set;

                        foreach (KeyValuePair<string, string> d_Char in char_UUID_Set)
                        {
                            if (d_Char.Value == characteristicUUID)
                            {
                                if (d_Char.Value == General_Information_Service.NumberOfUsageHours_CharUUID ||
                                    d_Char.Value == General_Information_Service.NumberOfBatteryRechargeCycles_CharUUID ||
                                    d_Char.Value == General_Information_Service.ErrorLog_CharUUID ||
                                    d_Char.Value == Scan_Data_Service.NumberOfStoredScans_CharUUID ||
                                    d_Char.Value == Scan_Data_Service.RequestStoredScanIndicesList_CharUUID ||
                                    d_Char.Value == Scan_Data_Service.GetStoredScanIndicesList_CharUUID
                                    )
                                {
                                    deprecated = true;
                                }
                        ...
                        return "Unknown Characteristic: " + characteristicUUID;
                    }
                }
                return "Unknown Service: " + serviceUUID;
            }
        }
