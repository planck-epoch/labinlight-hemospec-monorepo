  ...
  <q-page class="q-pa-md">
    <q-list bordered class="rounded-borders">
      <div class="row items-center crud-header">
        <div class="col-xs-12 col-sm-5">
          <q-item-label header class="row items-center">
            <div class="col">Analysis Bundles</div>
      ...
      <q-item>
        ...
        <q-item-section class="col-xs-4 col-sm-5">
      ...
      <q-item
        v-for="(analysisBundle, index) in analysisBundles"
        :key="analysisBundle.id"
        v-ripple
        clickable
        class="crud-item"
        :to="`/analysis_bundles/${analysisBundle.id}`"
      >
        <q-item-section avatar class="col-xs-2 col-sm-1">
        ...
        <q-item-section class="col-xs-4 col-sm-5">
          <q-item-label
            ...
            <q-icon v-if="analysisBundle.default" name="star" color="yellow" />
          </q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{ $formatFullDate(analysisBundle.updatedAt) }}
          </q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{ $formatFullDate(analysisBundle.createdAt) }}
          </q-item-label>
        </q-item-section>
        ...
        <q-item-section side class="col-xs-0 col-sm-2">
          <div class="text-grey-8 q-gutter-xs q-pr-sm">
            <q-btn
              ...
              round
              icon="delete"
              @click.native.prevent.stop="confirmDelete(analysisBundle.id)"
            />
            <q-btn
              class="gt-xs"
              size="12px"
              ...
              round
              icon="edit"
              :to="`/analysis_bundles/${analysisBundle.id}/edit`"
            />
          </div>
        </q-item-section>
      </q-item>
...
export default {
  name: "AnalysisBundlesList",
  mixins: [
    paginationMixin("analysisBundle", "id"),
    showMixin("analysisBundle"),
  ],
  computed: {
    ...mapGetters({
