<template>
  <q-page class="q-pa-md">
    ...
    >
      ...
      <q-item
        v-for="(analysisTest, index) in analysisTests"
        :key="analysisTest.id"
        v-ripple
        clickable
        class="crud-item"
        :to="`/analysis_tests/${analysisTest.id}`"
      >
        <q-item-section avatar class="col-xs-2 col-sm-1">
        ...
        <q-item-section class="col-xs-4 col-sm-5">
          <q-item-label>{{analysisTest.name}}</q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{$formatFullDate(analysisTest.updatedAt)}}
          </q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{$formatFullDate(analysisTest.createdAt)}}
          </q-item-label>
        </q-item-section>
        ...
        <q-item-section side class="col-xs-0 col-sm-2">
          <div class="text-grey-8 q-gutter-xs q-pr-sm">
            <q-btn
              ...
              round
              icon="delete"
              @click.native.prevent.stop="confirmDelete(analysisTest.id)"
            />
            <q-btn
              class="gt-xs"
              size="12px"
              ...
              round
              icon="edit"
              :to="`/analysis_tests/${analysisTest.id}/edit`"
            />
          </div>
        </q-item-section>
      </q-item>
...
<script>
  ...
  export default {
    name: 'AnalysisTestsList',
    mixins: [ paginationMixin('analysisTest', 'id'), showMixin('analysisTest') ],
    computed: {
      ...mapGetters({
