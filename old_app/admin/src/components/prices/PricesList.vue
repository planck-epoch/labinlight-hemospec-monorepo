  ...
  <q-page class="q-pa-md">
    ...
    >
      ...
      >
        ...
        <q-item-section class="col-xs-2 col-sm-3">
          <q-item-label>{{price.organization.name}}</q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-2 col-sm-2">
          <q-item-label>{{price.analysisBundle.name}}</q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{$formatFullDate(price.updatedAt)}}
          </q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{$formatFullDate(price.createdAt)}}
          </q-item-label>
        </q-item-section>
        ...
        <q-item-section side class="col-xs-0 col-sm-2">
          <div class="text-grey-8 q-gutter-xs q-pr-sm">
            <q-btn
              ...
              round
              icon="delete"
              @click.native.prevent.stop="confirmDelete(price.id)"
            />
            <q-btn
              class="gt-xs"
              size="12px"
...
<script>
  ...
  export default {
    name: 'PricesList',
    mixins: [ paginationMixin('price', 'id'), showMixin('price') ],
    computed: {
      ...mapGetters({
        prices: 'prices/all'
      }),
