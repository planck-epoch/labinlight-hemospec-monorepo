<template>
  <q-page class="q-pa-md">
    <q-list bordered class="rounded-borders">
      <div class="row items-center crud-header">
        <div class="col-xs-12 col-sm-5">
          <q-item-label header class="row items-center">
            <div class="col">DeviceConfigs</div>
      ...
      <q-item
        v-for="(deviceConfig, index) in deviceConfigs"
        :key="`cfg-${deviceConfig.id}`"
        v-ripple
        clickable
        class="crud-item"
        :to="`/deviceConfigs/${deviceConfig.id}`"
      >
        <q-item-section avatar class="col-xs-2 col-sm-1">
        ...
        <q-item-section class="col-xs-2 col-sm-3">
          <q-item-label>{{ "" + deviceConfig.id }}</q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-2 col-sm-3">
          <q-item-label>{{ deviceConfig.configName }}</q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{ $formatFullDate(deviceConfig.updatedAt) }}
          </q-item-label>
        </q-item-section>

        <q-item-section class="col-xs-3 col-sm-2">
          <q-item-label class="text-grey-8">
            {{ $formatFullDate(deviceConfig.createdAt) }}
          </q-item-label>
        </q-item-section>
        ...
        <q-item-section side class="col-xs-0 col-sm-2">
          <div class="text-grey-8 q-gutter-xs q-pr-sm">
            <q-btn
              ...
              round
              icon="delete"
              @click.native.prevent.stop="confirmDelete(deviceConfig.id)"
            />
            <q-btn
              class="gt-xs"
              size="12px"
              ...
              round
              icon="edit"
              :to="`/deviceConfigs/${deviceConfig.id}/edit`"
            />
          </div>
        </q-item-section>
      </q-item>
...
export default {
  name: "DeviceConfigsList",
  mixins: [paginationMixin("deviceConfig", "id"), showMixin("deviceConfig")],
  computed: {
    ...mapGetters({
